...
// (5 Phase-5 canonical + 1 post-audit email-job-purge + 1 fee-reminders.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 8 canonical crons (Phase 5 + post-audit + Frais & Scolarité + Stripe)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
