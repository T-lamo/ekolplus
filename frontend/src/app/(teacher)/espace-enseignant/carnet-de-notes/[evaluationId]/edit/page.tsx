...
export default function EditEvaluationPage() {
  ...
  }>(`/api/teacher/evaluations/${params.evaluationId}`, {
    skip: !user,
    onError: (err) => {
  ...
  async function onSave() {
    ...
    setLoadError(null);
    try {
      await api(`/api/teacher/evaluations/${value.id}`, {
        method: 'PATCH',
        body: {
      ...
      router.push(`/espace-enseignant/carnet-de-notes/${value.id}/saisie`);
    } catch (err) {
  ...
  async function onDelete() {
    ...
    setDeleting(true);
    try {
      await api(`/api/teacher/evaluations/${value.id}`, { method: 'DELETE' });
      toast(t('deletedToast'), 'success');
      router.push('/espace-enseignant/carnet-de-notes');
    } catch (err) {
      setLoadError(err instanceof ApiError ? err.message : tCommon('errors.network'));
      setDeleting(false);
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link
            href={`/espace-enseignant/carnet-de-notes/${value!.id}/saisie`}
