...
**Espace Enseignant (teacher portal).** `Teacher.userId` optionally links a `Teacher` row to a `User` for portal login, so one account can be teacher-linked without owning a full org membership. [frontend/src/lib/server/school.ts](frontend/src/lib/server/school.ts)'s `resolveMySchool()` is deny-by-default for this case: a MEMBER-role account linked to a portal-only `Teacher` (checked via its internal `isPortalOnlyAccount()`) gets `null` back, which locks down every existing `/api/school/*` route for free, with zero edits to those files. `resolveMySchoolIncludingTeacher()` is the deliberate opt-in for routes that must stay reachable by teacher-linked accounts (currently only `GET /api/school/timetable`, which then layers its own per-teacher scoping via `resolveMyTeacherProfile()`); note that scoping only triggers for `role === 'MEMBER'` today, so an ADMIN/OWNER who is also teacher-linked (a director who also teaches) still gets the full-school view there by design, and any teacher-portal-only page that must show that caller's own data regardless of role (e.g. the Emploi du temps screen under `/espace-enseignant`) has to fetch and pass its own `teacherId` explicitly rather than rely on server-side forcing. The teacher-facing surface itself lives under `/api/teacher/*` (`GET /api/teacher/me`, `GET /api/teacher/classes/[classSubjectId]`, `GET /api/teacher/classes/homeroom/[classId]`) and the `/espace-enseignant/*` page routes, all served by the same Next.js app behind the same auth cookies.
