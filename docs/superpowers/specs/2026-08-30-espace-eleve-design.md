...
A parallel, same-day design (`2026-08-30-espace-enseignant-design.md`) solves the structurally identical problem for teachers; its Phase 1 implementation plan (`docs/superpowers/plans/2026-08-30-espace-enseignant-phase1.md`, written and committed 2026-08-30, session `ekolplus2-e4`) has been read in full to ground this section in the actual planned code rather than a verbal description. Both specs converge on the same underlying mechanism:
...
This design reuses the following **generic, portal-type-agnostic pieces** the teacher-portal plan builds, rather than duplicating them:

- The outbox event kind `email.portal_invite` (payload: `{ to, code, expiresAt, portalLabel }`), with its `case` in `outbox/dispatcher.ts` — the student side reuses this case as-is; **no new dispatcher edit needed**.
- The template factory `portalInviteEmail({ code, email, expiresAt, portalLabel }): EmailTemplate` in `auth/email-templates.ts`.
- The `createPortalInvite(params): Promise<{ok:true,userId:string}|{ok:false,error:'EMAIL_ALREADY_IN_USE'}>` helper (new file `lib/server/portal-invite.ts`), taking `{ schoolId, organizationId, email, inviteType, portalLabel, expiresInMs, createOrgMembership, linkExisting }`. The student invite route calls it with `inviteType: 'STUDENT_INVITE'`, `createOrgMembership: false`, and a `linkExisting` callback that sets `Student.userId`.
- `resolveMySchool()`'s `isPortalOnlyAccount(userId, schoolId)` deny-by-default check (currently querying only `Teacher`) — this design extends that same function to also query `Student` once `Student.userId` exists. This is the one shared-file edit this design still performs, landing as an additive extension to an already-established extension point, sequenced after Phase 1 ships.

**Correction found while reading the teacher-portal plan in full**: the accept page and accept route are **not generic**, contrary to an earlier verbal description of this mechanism. The plan's Task 8/9 hardcode `frontend/src/app/(auth)/definir-mot-de-passe/page.tsx` to call `POST /api/auth/teacher-invite/accept` specifically, redirect to `/espace-enseignant` on success, and use `SetPassword`-namespace copy that says "teacher portal" — none of it parameterized by portal type. Only the deep primitives listed above are truly shared; the page/route pair is per-portal, following the same *pattern* (same validation, rate-limiting, and cookie-issuing shape as `/reset-password`) but not the same *code*. This design therefore builds its own analogous pair — a distinct page path (e.g. `frontend/src/app/(auth)/definir-mot-de-passe-eleve/page.tsx`, exact name to confirm) and `POST /api/auth/student-invite/accept` — modeled directly on the teacher plan's Tasks 8-9, rather than editing their page to add portal-type branching.

**Key difference from the teacher case**: a student-linked account gets **no `OrganizationMember` row at all** (a teacher-linked account gets one with role `MEMBER`, because teachers legitimately need some shared-route access) — the student invite route calls `createPortalInvite()` with `createOrgMembership: false`. This means a student account fails every existing `/api/school/*` staff route "for free" via `resolveMySchool()` returning `null` for non-members, exactly like today — no defense-in-depth lockdown sweep needed for students, unlike the teacher case. Every route the student portal needs is a brand-new `/api/student/*` endpoint; nothing pre-existing needs to change behavior.
...
2. ~~`createPortalInvite()` and `OrganizationMember`~~ — **Resolved 2026-08-30**: the helper now takes a `createOrgMembership: boolean` flag; the student invite route calls it with `createOrgMembership: false`, so no `OrganizationMember` row is created, consistent with this spec's no-lockdown-needed design.
...
**Routing after login**: the teacher plan's Task 10 adds `isTeacherOnly: boolean` to `GET /api/auth/me`'s response and Task 11 wires `login/page.tsx` to `isPlatformStaff ? '/admin' : me?.isTeacherOnly ? '/espace-enseignant' : '/dashboard'`. This design adds its own `isStudentOnly: boolean` to the same `/api/auth/me` response (via `resolveMyStudentProfile`, mirroring how `isTeacherOnly` is computed) and extends the same redirect chain with one more branch: `me?.isStudentOnly ? '/eleve' : '/dashboard'` as the final fallback. Also mirrors the teacher plan's Task 11 Step 4 belt-and-suspenders guard: `(school)/layout.tsx` bounces `user.isStudentOnly` accounts to `/eleve`, alongside its existing `isTeacherOnly` bounce.

## i18n

New message namespaces for genuinely new screens only: `EleveHome`, `EleveTimetable`, and a namespace for the new accept-invite page (e.g. `SetPasswordEleve` — the teacher plan's own `SetPassword` namespace is teacher-copy-specific per the "Correction" above, so this is a sibling, not a reuse). Everything else keeps the namespaces already used by the reused tab components (no new keys needed there). Invite-state strings fold into the existing `Eleves` namespace (mirrors the teacher plan folding its own into `Enseignants`). All three locales (fr/en/ht), matching `locales.test.ts`'s key-parity enforcement.
...
Phase 1 does not start until the teacher-portal work's own Phase 1 ships (`createPortalInvite()`, `email.portal_invite` outbox case, `portalInviteEmail()`, `isPortalOnlyAccount()` extension point, `isTeacherOnly` on `/api/auth/me`, all landed and merged — the `/definir-mot-de-passe` page itself does NOT need to land first, since this design builds its own separate accept page/route per the "Correction" above).

1. **Foundation**: schema change (`Student.userId`), `resolveMyStudentProfile`, `requireStudent`, `isStudentOnly` added to `/api/auth/me` (alongside the teacher plan's `isTeacherOnly`), the student invite route calling `createPortalInvite()`, the `POST /api/auth/student-invite/accept` route + `/definir-mot-de-passe-eleve` page, extending `isPortalOnlyAccount()` to also check `Student`, the `login/page.tsx` and `(school)/layout.tsx` redirect branches, a bare `/eleve` home page.
