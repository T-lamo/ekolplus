...
- **Outils :** workspace pnpm, Vitest (2220 tests), ESLint 9 flat config, Prettier, Node ≥ 20
...
`pnpm test` lance la suite Vitest (2220 tests, focalisée sur `lib/server/**` et les invariants critiques — pas de framework E2E en v1). `pnpm smoke:auth` vérifie le parcours auth complet contre une instance `pnpm dev` qui tourne.

## Contribuer

Voir [CONTRIBUTING.md](CONTRIBUTING.md) pour les conventions de branche, de commit et de test.
...
UNLICENSED — projet privé. Voir [LICENSE.md](LICENSE.md).
