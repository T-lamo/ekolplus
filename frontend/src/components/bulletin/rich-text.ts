...
// element names, text content, `style.textAlign` and the `align` attribute
// are read. Everything else (attributes, unknown elements, scripts) is
...
export interface DomNodeLike {
  ...
  style?: { textAlign?: string | undefined } | undefined;
}
...
function pushText(runs: RichRun[], text: string, marks: RichMark[]): void {
  if (!text) return;
  const m = normalizeMarks(marks);
  const last = runs[runs.length - 1];
  if (last && sameMarks(last.marks, m)) {
    last.text += text;
    return;
  }
  runs.push(m ? { text, marks: m } : { text });
}
...
function collectInline(node: DomNodeLike, marks: RichMark[], runs: RichRun[]): void {
  if (node.nodeType === TEXT_NODE) {
    pushText(runs, (node.textContent ?? '').replace(/\r/g, ''), marks);
    return;
  }
  ...
  const next = [...marks];
  if (tag === 'B' || tag === 'STRONG') next.push('bold');
  if (tag === 'I' || tag === 'EM') next.push('italic');
