...
// Consumes a STUDENT_INVITE code (Task 6's POST /api/auth/student-invite/accept)
// — the link a student lands on from their invite email. Structural mirror
// of /definir-mot-de-passe (teacher-portal's own accept page): accepting the
// invite establishes a session directly (all three auth cookies issued on
// success), so this page just refreshes AuthContext and drops the student
// straight into their portal — no separate "done" screen needed.
//
// The email link carries ?email=&code= so both arrive pre-filled — the
// student still has to choose a password and press submit themselves (no
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        // each one against the real SetPasswordEleve.errors.* keys at
        // compile time; a dynamically built key would bypass that check
        // (see frontend/src/app/login/page.tsx for the same reasoning).
        switch (err.code) {
